namespace :dlibworkflow do
  ...
  desc "Create exam papers from Fedora 3 repository"
  task create_exam_papers: :environment do
    collection_id = 'york:23505'
    # get children: http://dlib.york.ac.uk/solr/select/?q=rdf.rel.isMemberOf:%22york:23505%22&fl=dc.identifier

    # check if a collection, calling recursive method, otherwise, creating exam paper object



  end

  def create_exam_papers_within_a_collection(collection_pid)

  end

  desc "testing copy exam papers from Fedora 3 to Fedora 4"
  task test_copy_exam_paper: :environment do
    pid = 'york:926876'
    copy_exam_paper_from_f3_to_f4(pid)
  end

  # Copy Fedora 3 exam paper object to Fedora 4
  def copy_exam_paper_from_f3_to_f4(pid)
    server_url = ENV['OLD_FEDORA_SERVER_URL']
    path       = ENV['OLD_FEDORA_FOXML_PATH'].dup.gsub! '__PID__',pid
    ...
    conn.basic_auth(ENV['OLD_FEDORA_ADMIN'], ENV['OLD_FEDORA_PASSWORD'])
